"""Dialog for Docker/Kubernetes export configuration."""
...
class DockerExportDialog(QDialog):
    """Collect parameters for the Docker/K8s export bundle."""

    def __init__(self, parent=None) -> None:
        super().__init__(parent)
        self.setWindowTitle(t("docker_export_title"))
        ...
        # Gmail app-password + inline "test" button
        ...
        self._domain.setPlaceholderText("gallery.example.com")
        form.addRow(t("docker_export_domain"), self._domain)

        # Cluster issuer
        self._issuer = QLineEdit("letsencrypt-prod")
        form.addRow(t("docker_export_cluster_issuer"), self._issuer)
    ...
    def _browse_csv(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self,
            t("docker_export_csv_dialog_title"),
            str(Path.home()),
    ...
    def _validate(self) -> None:
        ...
        if not self._domain.text().strip():
            errors.append(t("docker_export_err_domain"))
        if errors:
            QMessageBox.warning(self, t("docker_export_title"), "\n".join(errors))
            return
    ...
    def cluster_issuer(self) -> str:
        return self._issuer.text().strip() or "letsencrypt-prod"
